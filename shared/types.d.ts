...
export type ViewerEffectType = 'drop_health' | 'drop_ammo' | 'drop_shield' | 'spawn_laser' | 'spawn_minion' | 'spawn_mine' | 'gravity_flip' | 'slow_motion' | 'spawn_turret' | 'rain_coins';
...
export interface EffectPricing {
}
export declare const EFFECT_PRICING: EffectPricing;
export interface ServerToClientEvents {
    ...
    roundEnd: (winner: Player | null) => void;
    anchorMatchCreated: (event: {
        matchId: string;
        creator: string;
        timestamp: number;
    }) => void;
    anchorPurchase: (event: {
        buyer: string;
        match_addr: string;
        effect_type: number;
        amount: string;
